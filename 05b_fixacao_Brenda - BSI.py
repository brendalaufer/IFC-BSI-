...
def maior_ponta(nums):
    ...
    for i in nums:
        maior_valor = nums[0] if nums [0] > nums[-1] else nums[-1]
        lista.append(maior_valor)
    return lista
...
def meio_do_caminho(a, b):
    ...
    """
    return [a[round(len(a) / 2.5)], b[round(len(b) / 2.5)]]
...
def gago(texto):
    ...
    """
    texto, x = texto.split(' '), 0
    while(x < len(texto)):
        texto[x] = texto[x][0]+'-'+texto[x] 
        x+=1
    return ' '.join(texto)
...
def nomes_pontas_e_iniciais_do_meio(n):
    ...
    for i in n[1:-1]:
        if i[0].isupper():
            sobrenome.append(i[0])
    return (n[0] + ' '.join(sobrenome) + ' ' + n[-1]).upper()
...
def tres_maiusculas(texto):
    ...
    for index, item in enumerate(texto):
        if item.isupper() and item == texto[index+1] and item == texto[index+2]:
            return index
...
def im_pares_unicos(lista):
    ...
    """
    lista_impar, lista_par = [], []
    for i in lista:
        if False == (i in lista_par) and False == (i in lista_impar): lista_par.append(i) if (i % 2 == 0) else lista_impar.append(i)
